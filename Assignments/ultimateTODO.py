# Zachary Murphy             11-7-2025
# Assignment #5, Lecture Section 4, Lab Section F
# This code will let the user store data in a todo list, and allow them to move the data around to whichever list best fits the state of the data.
...
def checkItem(item, todoList):
    ...
    for keys in todoList:
        for i in range(len(todoList[keys])):
            if todoList[keys][i] == item:
                itemFound = True
                keyName = keys
                index = i
...
def addItem(item, toList, todoList):
    ...
    if itemFound:
        print(f"ERROR: {item} was found in {keyName} at index {index}")
    else:
...
def runApplication(todoList):
    ...
    while True:
        ...
        if choice == "a":
            ...
            item = str(input("Enter the item you would like to add: "))
            ...
            if checkIfListEmpty(todoList):
                print("ERROR: ALL lists are empty.")
            else:
                item = str(input("Enter the item you would like to move: "))
                while checkItem(item, todoList) == False:
                    item = str(input("ERROR: Item not found, enter a different item: "))

                toList = str(input("Enter the list you would like to move the item to: "))
                running = True
                ...
                for keys in todoList:
                    if toList == keys:
                        ...
                        break
                while running:
                    toList = str(input("ERROR: List doesn't exist, enter a different list: "))
                    for keys in todoList:
                        if toList == keys:
                            running = False
            ...
            if checkIfListEmpty(todoList):
                ...
                item = str(input("Enter the item you would like to delete: "))
                itemFound, todoList = deleteItem(item, todoList)
            printTODOList(todoList)
